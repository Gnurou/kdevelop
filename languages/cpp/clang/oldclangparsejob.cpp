...
class CLangContextBuilder : public AbstractContextBuilder<CXCursor, QString>
{
public:
    CLangContextBuilder() {}
    ...
    CursorInRevision toCursor(const CXSourceLocation &cursor)
    {
...
bool CLangDeclBuilder::VisitTypeDecl(clang::TypeDecl *decl)
{
    ...
    qDebug() << "type" << decl->getQualifiedNameAsString().c_str();
    ...
    //qDebug() << astContext->getCanonicalType(decl->getTypeForDecl());
    //qDebug() << astContext->getCanonicalType(decl->getTypeForDecl())->getTypeClassName();
